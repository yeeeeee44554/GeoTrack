...
function PageHeader({ kicker, title, description, action }) {
  return <div className="page-header"><div><div className="page-kicker">{kicker}</div><h1>{title}</h1>{description ? <p>{description}</p> : null}</div>{action}</div>
}
...
function QualityView({ data, onRefresh }) {
  ...
  const processingMode = data.fullSummary?.summary?.processing_mode ?? (data.mode === 'demo' ? 'demo-seed' : 'processed-demo')
  const localFullFallback = processingMode === 'local-full-serving-index'
  const statusLabel = { queued: '排队中', running: '运行中', completed: '已完成', failed: '失败' }
  const sourceLabel = localFullFallback ? '本地 full-index SQLite 回退' : processingMode === 'demo-seed' ? 'demo seed 回退' : 'processed serving payload'
  return <div className="page-content"><PageHeader kicker="PIPELINE OPS / 05" title="让每一批数据都有迹可循" description="查看数据质量、处理批次与 Spark 任务入口，保证结果可复现、可答辩。" action={<button className="primary-button" onClick={handleRun} disabled={running || ['queued', 'running'].includes(job?.status)}>{running ? <RefreshCw className="spin" size={16} /> : <Play size={16} />}{running ? '提交中' : job?.status === 'running' ? '运行中' : '运行挖掘任务'}</button>} /><div className="dashboard-grid quality-grid"><section className="panel"><SectionHeading eyebrow="QUALITY REPORT" title="清洗摘要" meta={sourceLabel} /><div className="quality-metrics"><div><span>有效点</span><strong>{fmt(quality.valid_points)}</strong><small>保留进入特征工程</small></div><div><span>重复点</span><strong>{fmt(quality.duplicate_points)}</strong><small>连续重复坐标时间点</small></div><div><span>时间断点</span><strong>{fmt(quality.time_gap_segments)}</strong><small>超过 30 分钟</small></div><div><span>停留点</span><strong>{fmt(quality.stay_point_count)}</strong><small>200m / 20min 规则</small></div></div><div className="quality-progress"><div><span>点级有效率</span><strong>99.3%</strong></div><div className="progress-track"><i style={{ width: '99.3%' }} /></div></div></section><section className="panel"><SectionHeading eyebrow="RUNBOOK" title="处理链路" meta="四层架构" /><div className="pipeline-list"><div className={`pipeline-item ${localFullFallback ? 'active' : 'done'}`}><span>01</span><div><strong>{localFullFallback ? 'PLT / local full scan' : 'PLT / HDFS raw'}</strong><small>{localFullFallback ? '逐文件有界内存扫描（本机已验证）' : '原始轨迹文件落盘'}</small></div><i>{localFullFallback ? '●' : '✓'}</i></div><div className={`pipeline-item ${localFullFallback ? 'active' : 'done'}`}><span>02</span><div><strong>{localFullFallback ? 'Spark clean · 待目标机' : 'Spark clean'}</strong><small>{localFullFallback ? 'Parquet 与 MLlib 仍需 Docker/Spark 证据' : '清洗、距离、质量报告'}</small></div><i>{localFullFallback ? '●' : '✓'}</i></div><div className={`pipeline-item ${localFullFallback ? '' : 'active'}`}><span>03</span><div><strong>{localFullFallback ? 'MobilityDB serving · 待目标机' : 'MobilityDB serving'}</strong><small>{localFullFallback ? '当前 API 使用本地回退索引' : '空间索引与时态轨迹'}</small></div><i>{localFullFallback ? '○' : '●'}</i></div><div className="pipeline-item done"><span>04</span><div><strong>REST / React</strong><small>查询、地图和图表展示</small></div><i>✓</i></div></div><div className={`job-message ${job?.status === 'failed' ? 'job-failed' : ''}`}>{job ? <><strong>{statusLabel[job.status] || job.status}</strong> · {message}{job.started_at ? <small>开始 {job.started_at.slice(11, 19)} UTC</small> : null}</> : message || '批处理任务可以在 Docker/Spark 环境中替换为真实 spark-submit。'}</div></section></div><section className="panel implementation-note"><Server size={20} /><div><strong>课程答辩提示</strong><p>演示时说明：HDFS 保存 raw/curated Parquet，Spark 生成停留点和聚类结果，MobilityDB 负责空间检索，FastAPI 只查询预计算服务表，避免网页请求阻塞批处理。当前页面会明确标出本机回退索引与目标机待验证项。</p></div><button className="ghost-button" onClick={onRefresh}>刷新数据</button></section></div>
}
